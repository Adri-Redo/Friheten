...
public class GUIModificarPersonal extends JFrame implements Observer {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (InfoPanel ip : atributos) {
					...
					case "Id":
						...
						String idPersonal = ip.tf.getText().trim();
						int id = 0;
						try {
							id = Integer.parseInt(idPersonal);

							if (id <= 0) {
								JOptionPane.showMessageDialog(null, "El identificador debe ser mayor que 0.", "ERROR",
							...
							JOptionPane.showMessageDialog(null,
									"El identificador debe ser un numero entero mayor que 0, sin letras.", "ERROR",
									JOptionPane.ERROR_MESSAGE);
							validar = false;
						}
						atributosValues.add(id);
					...
					case "Nombre":
						...
						if (validar && !nombre.matches("^[a-zA-Z]+$")) {
							JOptionPane.showMessageDialog(null, "El nombre debe de contener unicamente letras", "ERROR",
									JOptionPane.ERROR_MESSAGE);
							validar = false;
						}
						if (validar) {
					...
					case "Apellido":
						...
						if (validar && !apellido.matches("^[a-zA-Z]+$")) {
							JOptionPane.showMessageDialog(null, "El apellido debe de contener unicamente letras",
									"ERROR", JOptionPane.ERROR_MESSAGE);
							validar = false;
						}
						if (validar) {
					...
					case "IdNave":
						...
						try {
							...
							if (idNave <= 0) {
								JOptionPane.showMessageDialog(null, "El identificador de nave debe ser mayor que 0.",
										"ERROR", JOptionPane.ERROR_MESSAGE);
								validar = false;
							}
							...
							JOptionPane.showMessageDialog(null,
									"El identificador de nave debe ser un numero entero mayor que 0, sin letras.",
									"ERROR", JOptionPane.ERROR_MESSAGE);
							validar = false;
						}
				...
				if (validar) {
					...
					if (type.equals("Jefe")) {
						//desde gui: id,nif,nombre,apellido,nave,nommina,responsabilidades
						//Transfer: nif,nombre,apellido,id,nave,activo,nomina,responsabilidades,bonificaciones
						Context contexto = new Context(Events.ALTA_PERSONAL_OK_VIEW,new TJefe((String) atributosValues.get(1), (String) atributosValues.get(2),
								(String) atributosValues.get(3), (Integer) atributosValues.get(0),
								(Integer) atributosValues.get(4), true,(Integer)atributosValues.get(5)
								,(Integer)atributosValues.get(6),0));
						Controller.getInstance().handle(contexto);
					} else if (type.equals("Empleado")) {
						//desde gui: id,nif,nombre,apellido,nave,cargo,nommina
						//Transfer: nif,nombre,apellido,id,nave,cargo,activo,nomina,bonificaciones
						Context contexto = new Context(Events.ALTA_PERSONAL_OK_VIEW,new TEmpleado((String) atributosValues.get(1), (String) atributosValues.get(2),
								(String) atributosValues.get(3), (Integer) atributosValues.get(0),
								(Integer) atributosValues.get(4), (String) atributosValues.get(5), true,(Integer)atributosValues.get(6),
								0));
						Controller.getInstance().handle(contexto);
					}
