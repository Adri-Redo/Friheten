package Presentacion.Producto_Cafeteria.ConsultarProductoCafeteriaPorAlergerno;
...
public class GUIConsultarProductoCafeteriaPorAlergeno extends JFrame
    implements Observer {
    ...
    private JTextField txtAlergenoId;
    private JLabel statusLabel;

    public GUIConsultarProductoCafeteriaPorAlergeno() {
        super("CONSULTAR PRODUCTOS POR ALERGENO");
        initGUI();
    }
    ...
    private void initGUI() {
        ...
        JLabel titulo = new JLabel(
            "Consultar Productos por Alergeno",
        ...
        JLabel idLabel = new JLabel("ID del Alergeno:");
        idLabel.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        txtAlergenoId = new JTextField(10);
        txtAlergenoId.setFont(new Font("Segoe UI", Font.PLAIN, 14));

        JButton consultarButton = createStyledButton("Consultar");
        JButton cancelButton = createStyledButton("Cancelar");

        searchPanel.add(idLabel);
        searchPanel.add(txtAlergenoId);
        ...
        consultarButton.addActionListener(e -> {
            String idText = txtAlergenoId.getText().trim();
            if (idText.isEmpty()) {
                showError("El ID del alergeno no puede estar vacío.");
            } else {
                if (isValidPositiveInteger(idText, "El ID del alergeno")) {
                    try {
                        Integer idAlergeno = Integer.parseInt(idText);
                        ...
                        Controller
                            ...
                            .handle(
                                new Context(
                                    Events.CONSULTAR_PRODUCTO_POR_ID_ALERGENO_OK_VIEW,
                                    idAlergeno
                                )
                            );
    ...
    public void actualizar(Context context) {
        if (
            context != null &&
            context.getEvent() == Events.CONSULTAR_PRODUCTO_POR_ID_ALERGENO_VIEW
        ) {
            tableModel.setRowCount(0);
            ...
            try {
                ...
                if (data instanceof Set) {
                    ...
                    if (productList.isEmpty()) {
                        statusLabel.setText(
                            "No se encontraron productos para ese alergeno."
                        );
                        showInfo(
                            "No se encontraron productos activos para el alergeno especificado."
                        );
                    } else {
                    ...
                    showInfo(
                        "No se recibieron datos para el alergeno especificado."
                    );
                } else {
                ...
                System.err.println(
                    "Error inesperado en GUIConsultarProductoCafeteriaPorAlergeno.actualizar: " +
                    e.getMessage()
                );
                e.printStackTrace();
            }
            ...
            context != null &&
            context.getEvent() == Events.CONSULTAR_PRODUCTO_POR_ID_ALERGENO_KO_VIEW
        ) {
            String errorMsg = (context.getData() instanceof String)
                ? (String) context.getData()
                : "Error desconocido al consultar el alergeno.";
