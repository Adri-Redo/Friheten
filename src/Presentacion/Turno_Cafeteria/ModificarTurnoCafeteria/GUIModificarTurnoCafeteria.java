 ...
 * 
 */
package Presentacion.Turno_Cafeteria.ModificarTurnoCafeteria;
...
*/
public class GUIModificarTurnoCafeteria extends JFrame implements Observer, Serializable {
	/**
	 * 
	 */
	...
	public GUIModificarTurnoCafeteria() {
	...
	*/
	private void initGUI() {
		...
		JLabel titleLabel = new JLabel("Modificar un turno en la base de datos");
		title.add(titleLabel);
		mainPanel.add(title, BorderLayout.NORTH);

		JPanel pregunta_id = new JPanel();
		JLabel idLabel = new JLabel("Id: ");
		JTextField id = new JTextField(10);
		pregunta_id.add(idLabel);
		pregunta_id.add(id);
		...
		ok.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					Integer idNum = Integer.parseInt(idText);
					if (idNum <= 0) {
						valido = false;
				...
				if(valido && !nombre.matches("^[a-zA-Z0-9]*[a-zA-Z0-9]+[a-zA-Z0-9]*$")) {
					JOptionPane.showMessageDialog(null,
				...
				if(valido && !numHoras.matches("^(1[0-9]|2[0-4]|[1-9])$")) {
					...
					Controller.getInstance().handle(new Context(Events.MODIFICAR_TURNO_OK_VIEW, tTurno));
				}
				else 
					Controller.getInstance().handle(new Context(Events.TURNO_KO_VIEW, null));
