...
public class Command_DesvincularHabilidadOk_View implements Command {

	@Override
	public Entry<Integer, Context> execute(Context context) {
		THabilidadPersonal habPer = (THabilidadPersonal) context.getData();
		int resultado = FactoryNeg.getInstance().generateSAHabilidad().deleteHabilidadPersonal(habPer);
		Context new_context = new Context();

		if (resultado == -1) {// NO se ha podido crear
			new_context.setEvent(Events.ERROR_HABILIDAD_NO_VINCULADA_A_PERSONAL);
			...
			new_context.setEvent(Events.EXITO_HABILIDAD_DESVINCULAR);
