...
*/
public class Command_UpdateAlergenoOk_View implements Command {
	/** 
	* (non-Javadoc)
	 * @return 
	...
	*/
	...
	public Entry<Integer, Context> execute(Context context) {
	    ...
	    int resultado = FactoryNeg.getInstance().generateSAAlergeno().update(al);
	    Context new_context = new Context();
	    if (resultado == -1) {
	        new_context.setEvent(Events.ERROR_ALERGENO_NO_EXISTE);
	        new_context.setData(al.getIdAlergeno());
	    } else if (resultado == -2) {
	        new_context.setEvent(Events.ERROR_ALERGENO_MODIFICAR);
	        ...
	        new_context.setEvent(Events.EXITO_ALERGENO_MODIFICAR);
